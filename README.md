...
| 이유 | 조치 |
|------|------|
| `개요`, `테스트환경 셋팅`, `상시 업무` 등 | 정상 — CSV 아닌 관리용 탭 |
| `[TEST]`, `[HEIMDALL]` 등 접미사 있는 탭 | 정상 — 별도 관리 탭 |
| `(서브문서에 탭 없음)` | 근본 시트 서브문서에 해당 탭이 없음 → 수동 확인 필요 |
| 탭 이름만 있고 괄호 없음 | 근본 시트 `기본` 탭에 D열 링크 없음 → 링크 추가 필요 |
...
**2. config 파일 작성**
`config/tablecsv.json` 파일을 아래 형식으로 작성합니다:
```json
{
...
> ⚠️ `config/tablecsv.json`은 절대 GitHub에 올리지 마세요. `.gitignore`에 등록되어 있습니다.
> 형식 참고: `config/tablecsv.example.json`

**3. GitHub 토큰 권한**
토큰은 본인 fork 레포(`fork_owner/lib9c`)에 **write** 권한이 있어야 합니다.
