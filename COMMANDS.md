...
**설명**: 아레나 보상 공지용 PPTX (2슬라이드: Odin + Heimdall)를 생성하고 파일로 전송한다.

**사용법**:
```
arena <odin_type> <odin_num> <odin_start> <odin_end> <heim_type> <heim_num> <heim_start> <heim_end>
```

**예시**:
```
arena Championship 16 17889224 18040423 Season 21 9412781 9563980
```

- `odin_type` / `heim_type`: `Championship` 또는 `Season`
- 상금 자동 결정: Championship → 500,000 / Season → 400,000
- 날짜 계산용 기준 블록은 스크립트에 고정 (별도 입력 불필요)

**동작**:
1. 상금·날짜 자동 계산
2. 보상 테이블 재계산
3. 2슬라이드 PPTX 생성 후 전송
...
**스크립트**: `scripts/arena` → `scripts/create-arena-pptx.py`
